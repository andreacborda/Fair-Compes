...
export default function App(){
  ...
  // AUTH & PAYWALL SYSTEM
  const [searchCount,setSearchCount]=useState(()=>{try{return parseInt(localStorage.getItem("fc_searches")||"0");}catch{return 0;}});
  const [userEmail,setUserEmail]=useState(()=>{try{return localStorage.getItem("fc_email")||"";}catch{return "";}});
  const [isPaid,setIsPaid]=useState(()=>{try{return localStorage.getItem("fc_paid")==="true";}catch{return false;}});
  const [showLogin,setShowLogin]=useState(false);
  const [showPaywall,setShowPaywall]=useState(false);
  const [showTutorial,setShowTutorial]=useState(false);
  const FREE_LIMIT=5;

  // Paid users - these emails have unlimited access
  const PAID_EMAILS=["andrea9522@gmail.com"]; // Add paid user emails here

  const handleLogin=(email)=>{
    setUserEmail(email);
    try{localStorage.setItem("fc_email",email);}catch{}
    setShowLogin(false);
    // Check if this email has paid access
    if(PAID_EMAILS.includes(email.toLowerCase())){
      setIsPaid(true);
      try{localStorage.setItem("fc_paid","true");}catch{}
    }else{
      setShowPaywall(true);
    }
  };
  ...
  const handleSearch=()=>{
    ...
    try{localStorage.setItem("fc_searches",String(newCount));}catch{}
